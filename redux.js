...
function inject(context) {
	...
	/**
	 * Tracks and stores various data for redux, like events, elements, and included
	 * files.
	 */
	redux.data = {
		...
		types: {
			...
			Filesystem: ['TempDirectory', 'TempFile'],
			Map: ['Annotation'],
			...
			Platform: ['UUID'],
			Stream: ['Stream'],
			UI: ['2DMatrix', '3DMatrix', 'ActivityIndicator', 'AlertDialog', 'Animation', 'Button', 'ButtonBar', 'CoverFlowView', 'DashboardItem', 'DashboardView', 'EmailDialog', 'ImageView', 'Label', 'MaskedImage', 'Notification', 'OptionDialog', 'Picker', 'PickerColumn', 'PickerRow', 'ProgressBar', 'ScrollView', 'ScrollableView', 'SearchBar', 'Slider', 'Switch', 'Tab', 'TabGroup', 'TabbedBar', 'TableView', 'TableViewRow', 'TableViewSection', 'TextArea', 'TextField', 'Toolbar', 'View', 'WebView', 'Window', 'ListView', 'ListSection', 'ListItem', 'NavigationWindow', 'SlideMenu']
		},
		defaults: {
			byID: {},
			byRclass: {},
	...
	redux.fn = redux.prototype = {
		/**
		 * Returns the objects that match your selector, or the root redux object if you
		 * did not provide a selector. Note that only
		 * objects created by redux constructors can be selected (ex use new Label()
		 * instead of Ti.UI.createLabel()).
		...
		/**
		 * Turns a string of RJSS into JavaScript that can be safely evaluated. RJSS is a
		 * way to customize JavaScript
		...
		/**
		 * Includes and parses one or more RJSS files. Styles will be applied to any
		 * elements you create after calling this.
		...
		includeRJSS: function() {
			for (var i = 0, l = arguments.length; i < l; i++) {
				...
				try {
					...
					context.error('RJSS "' + arguments[i] + '" has syntax errors:' + e);
		...
		/**
		 * Adds an event binder that can bind listen events or fire events, similar to
		 * how jQuery's events stack works.
		 * @param {Object} event
		 */
		...
		applyOrientation: function(obj, orientation, args, override) {
			...
			mergeObjects(obj, styles, override);
			// obj.applyProperties(styles)
			// for (var index in styles) {
			// obj[index] = styles[index];
			// }
		},
		/**
		 * Adds a natural constructors for all the different things you can create with
		 * Ti, like Labels,
		 * LoginButtons, HTTPClients, etc. Also allows you to add your own natural
		 * constructors.
		 *
		 * @param context The context to add this constructor to ("this" would be a good
		 ...
		 * @param constructorName The desired constructor name; defaults to type. Generic
		 * styles will use this.
		 */
		 ...
		 * @param context The context to add this constructor to ("this" would be a good
		 * thing to pass in here)
		 * @param namespace The namespace under Ti that the object will be created in
		 * (like UI, as in Ti.UI)
		 * @param type The type of the constructor (like Label or Button)
		 * @param constructorName The desired constructor name; defaults to type. Generic
		 * styles will use this.
	...
	for (var i3 in redux.data.types) {
		...
		if (redux.data.types.hasOwnProperty(i3)) {
			for (var j3 = 0, l3 = redux.data.types[i3].length; j3 < l3; j3++) {
				// iterate over types within parent namespace (Label, LoginButton, HTTPClient,
				// etc)
	...
	/**
	 * Expose the applyStyle function to selector based redux usages --
	 * $(view).applyStyle() etc.
	 * @param type
	 * @param args
	 */
